...
function Transform(code = "") {
    ...
    //Match all component tags
    /*const _matchComponentTag = code.matchAll(/<hx\-app.*?>.*?<\/hx\-app>/gm);
    for(const _tag of _matchComponentTag) {
    
        //Pharse dom and replace it with methods
        const _dom = new JSDOM(_tag[0]);
        const _transformed = DOMPharse(_dom.window.document.body.children[0]);
        code = code.replace(_tag[0], _transformed);
...
function DOMPharse(_element = document.body) {
    ...
    for(var i = 0; i < _attribute.length; i++) {
        ...
        if(_start !== null && _start[0] == "@") {
            ...
            if(_start[1] == "[") {
                ...
                /*let _array = _value.match(/(?<!'|"|\s){[a-zA-Z0-9_$]+}(?!'|")/gm);
                if(_array !== null) {

                    for(const x of _array) {
                        _value = _value.replace(x, `'${x}'`);
                    };
    ...
    let _component = (_element.hasAttribute("args")) ? "component" : "node";
    ...
    let _code = `${_await} this.${_component}(${_name},${_child_code}${_attributeValue})`;
